...
public abstract class AbstractTableFormatter implements TableFormatter {
    ...
    private List<Integer> maxColWidths = new ArrayList<>();
    private List<Integer> maxRowHeights = new ArrayList<>();
    private List<List<CellData>> tableData = new ArrayList<>();
    ...
    public final TableFormatter addLine(String text) {
        ...
        List<CellData> row = tableData.get(tableData.size() - 1);

        if (row.size() <= 0) {
            throw new IllegalStateException("Cannot addLine() to empty row.  Use nextCell() first.");
        }

        CellData cell = row.get(row.size() - 1);
        ...
        int maxColWidth = maxColWidths.get(currentCol);

        if (text != null && text.length() > maxColWidth) {
            maxColWidths.set(currentCol, text.length());
        }
        ...
        int maxRowHeight = maxRowHeights.get(currentRow);

        if (cell.getLineCount() > maxRowHeight) {
            maxRowHeights.set(currentRow, cell.getLineCount());
        }
    ...
    public final TableFormatter nextRow() {
        tableData.add(new ArrayList<CellData> (columnCount));
    ...
    public final int getColumnWidth(int columnIndex) {
        ...
        return maxColWidths.get(columnIndex);
    }
    ...
    public final int getRowHeight(int rowIndex) {
        ...
        return maxRowHeights.get(rowIndex);
    }
    ...
    public final String[] getFormattedCell(int rowIndex, int columnIndex) {
        ...
        List<CellData> row = tableData.get(rowIndex);

        // Is there a cell at the specified row/col?
        if (row.size() > columnIndex) {
            CellData cell = row.get(columnIndex);
        ...
        for (int i = 0; i < lines.size(); ++i) {
            result.add(getFormattedLine(lines.get(i), cellWidth, align));
        }
